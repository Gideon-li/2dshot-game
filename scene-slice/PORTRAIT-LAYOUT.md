...
| 锚点 | 世界 position | 用途 |
| --- | --- | --- |
| 病人椅座位 | **(1280, 1280)** | `_seat_patient`；桌前南侧地面（V139.1） |
| 候诊凳 A / B | **(1920, 1280)** / **(2060, 1270)** | 右过道地面（V139.1） |
| 候诊旁位 C | **(1800, 1300)** | Patient2 home（V139.1） |
| 候诊凳 D | **(2180, 1260)** | Patient3 / WAIT_D（V139.1） |
| 江晚学徒锚点 | **(1220, 1080)** | 椅腿/桌下地面；**非** PhysicianChair 节点；V139.1 |
| 家具节点（只读） | Desk (1090,720) 等 | **不可当脚底**；见 `CLINIC-CONSULT-V139.md` |

> **V139.1：** 上表已过时。权威脚底见 **§11** / `CLINIC-CONSULT-V139.md`（江晚 (1220,1080)·坐诊 (1280,1280)·A–D 右过道）。

名字标签：脚底上方 `Vector2(-48, -display_h - 8)`，高 22，勿压四诊 HUD（诊室 HUD 在 `UI/Hud`，不在 L5）。

**换单人图时**：拆掉 `L5_characters/Patients` 合图依赖；`Patient0/1/2` 各挂一张 Texture；座位仍 tween 到 **(1280, 1280)**（V139.1）。
...
| | 诊室 | 问诊 |
| --- | --- | --- |
| 江晚 | 锚点 **(1220, 1080)**（椅腿/桌下地面），显示高约 **300～320** | 默认**不**占 PortraitFrame；最多角落小头像 64²（选做） |
| 当前病人 | 座位 **(1280, 1280)**，显示高 280～320 | PortraitFrame 主位，按 `current_patient_id` 换贴图 |
...
- 新候诊 home：`WAIT_D` / `Patient3` / `char_xiuniang`；坐标经 V139→**V139.1** 现为 **(2180, 1260)**（见 §11 / `CLINIC-CONSULT-V139.md`）。
- 坐诊 **(1280, 1280)**；四人共用同一 PortraitFrame，按 id 换贴图。
...
**本 addendum 脚底曾覆盖 V138；现已由 §11 V139.1 整表取代（含坐诊）。下表仅作历史对照。**
...
- **§10 脚底已由 §11 / `CLINIC-CONSULT-V139.md`（V139.1）取代。** 原 Look 偏移草案作废；勿再按 (1035,770)/(1040,590)/左走道 A–D 挂脚。

## 11. V139.1 addendum · 脚底纠偏 + L3 桌前遮挡

日期：2026-09-24  
权威全文：`scene-slice/CLINIC-CONSULT-V139.md`（V139.1 修订）· 示意 `scene-slice/waiting/CLINIC_CONSULT_V139.png`（镜 `ui/waiting/`，同文件名新内容）。  
别名摘要：`CLINIC-CONSULT-V139.1.md`。

**本 addendum 脚底整表覆盖 §10 / 原 V139（含坐诊）。** Look 偏移草案作废。

| 槽 | V139.1 脚底 | 热区中心 | vs V139 |
| --- | --- | --- | --- |
| 江晚 | **(1220, 1080)** | — | 原 (1035,770)；显示高 **300～320** |
| 坐诊 `_seat_patient` | **(1280, 1280)** | (1280, 1245) r=70 | 原 (1040,590)；ΔY 对江晚 =200 |
| A | **(1920, 1280)** | (1920, 1245) r=90 | 原 (400,400) → 右过道 |
| B | **(2060, 1270)** | (2060, 1235) r=90 | 原 (540,400) |
| C | **(1800, 1300)** | (1800, 1265) r=90 | 原 (280,430) |
| D | **(2180, 1260)** | (2180, 1225) r=90 | 原 (680,410) |

- **`L3_desk_front`**：桌前立面+桌沿独立 Sprite；**排序原点 Y=1160**，z≈4；脚 Y&lt;1160 画在桌后，&gt;1160 画在桌前。  
- 机位：idle/开诊默认 **`CAM_HERO (1280, 780)`** ≈0.67；单轴 nudge≤40。开诊可选 `CAM_ASK (1260, 1140)` ≈0.62。  
- 作业区禁入：**x∈[900,1500] ∩ y∈[960,1200]**。  
- 道具尺度沿 V139：药壶 ≤220×260；脉枕 ≤160×48；香炉高 ≤56。  
- 家具节点 Desk/Chair/Bench **不可当脚底**。
